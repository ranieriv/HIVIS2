...
struct DeviceConfig {
    ...
    // --- From config.json ---
    int    mqttPort;
    String mqttTopicPrefix;  // Kept for config, not used directly in publish topic
    int    mqttIntervalMs;
    int    mqttKeepaliveS;
...
// Local fallback device ID — MAC lower 4 bytes as hex
// The server may overwrite this with a full MAC-derived ID on provisioning.
inline String deriveDeviceId() {
    return "hivis-" + String((uint32_t)ESP.getEfuseMac(), HEX);
}

// Load config.json from LittleFS (non-secret parameters only)
...
// Load device identity and MQTT credentials from NVS
inline void loadNVS(DeviceConfig &cfg) {
    ...
    // Local fallback device ID if server hasn't assigned one yet
    if (cfg.deviceId.isEmpty()) cfg.deviceId = deriveDeviceId();
    if (cfg.deviceName.isEmpty()) cfg.deviceName = cfg.deviceId;
}

// Save identity params to NVS after portal completes
...
// Save provisioned MQTT credentials (received from server)
...
// Save last OTA check time (seconds since epoch, or millis-derived offset)
...
// Wipe entire "hivis" NVS namespace — factory reset
